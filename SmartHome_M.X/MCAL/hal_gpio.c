/*
 * File: hal_gpio.c
 * Author: Fathy.Ramadan
 * created on September 13, 2023, 4:31 PM 
 */
...
Std_ReturnType gpio_pin_direction_initialize(const pin_config_t *_pin_config){
    ...
    if(NULL == _pin_config || _pin_config->pin >= PORT_PIN_MAX_NUMBER || _pin_config->port >= PORT_MAX_NUMBER){
        ret = E_NOT_OK;
    }
    else{
...
Std_ReturnType gpio_pin_get_direction_status(const pin_config_t *_pin_config, direction_t *direction_status){
    Std_ReturnType ret = E_OK;
    if(NULL == _pin_config || NULL == direction_status || _pin_config->pin >= PORT_PIN_MAX_NUMBER || _pin_config->port >= PORT_MAX_NUMBER){
        ret = E_NOT_OK;
    }
    else{
...
/**
 * 
 * @param port pointer to port configurations @ref port_index_t
 * @param direction
 * @return Status of the function
 *          (E_OK) : The function done successfully
 *          (E_NOT_OK) : The function has issue to perform this action
